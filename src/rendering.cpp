...
void DrawCow(Vector3 pos, bool highlighted) {
    ...
    DrawCube((Vector3){pos.x, pos.y + 0.6f, pos.z}, 0.6f, 0.5f, 1.0f, cowBody);

    // Spots on body
    DrawCube((Vector3){pos.x + 0.2f, pos.y + 0.7f, pos.z + 0.1f}, 0.2f, 0.2f, 0.3f, cowSpots);
    DrawCube((Vector3){pos.x - 0.15f, pos.y + 0.65f, pos.z - 0.2f}, 0.15f, 0.15f, 0.25f, cowSpots);

    // Head
    DrawCube((Vector3){pos.x, pos.y + 0.7f, pos.z + 0.65f}, 0.35f, 0.35f, 0.3f, cowBody);

    // Snout/nose
    DrawCube((Vector3){pos.x, pos.y + 0.6f, pos.z + 0.85f}, 0.2f, 0.15f, 0.1f, cowPink);

    // Eyes
    DrawSphere((Vector3){pos.x - 0.12f, pos.y + 0.8f, pos.z + 0.75f}, 0.05f, eyeWhite);
    DrawSphere((Vector3){pos.x + 0.12f, pos.y + 0.8f, pos.z + 0.75f}, 0.05f, eyeWhite);
    DrawSphere((Vector3){pos.x - 0.12f, pos.y + 0.8f, pos.z + 0.78f}, 0.025f, eyeBlack);
    DrawSphere((Vector3){pos.x + 0.12f, pos.y + 0.8f, pos.z + 0.78f}, 0.025f, eyeBlack);

    // Ears
    DrawCube((Vector3){pos.x - 0.2f, pos.y + 0.85f, pos.z + 0.55f}, 0.1f, 0.05f, 0.1f, cowBody);
    DrawCube((Vector3){pos.x + 0.2f, pos.y + 0.85f, pos.z + 0.55f}, 0.1f, 0.05f, 0.1f, cowBody);

    // Legs (4 legs)
    float legHeight = 0.35f;
    DrawCube((Vector3){pos.x - 0.2f, pos.y + legHeight/2, pos.z + 0.35f}, 0.1f, legHeight, 0.1f, cowBody);
    DrawCube((Vector3){pos.x + 0.2f, pos.y + legHeight/2, pos.z + 0.35f}, 0.1f, legHeight, 0.1f, cowBody);
    DrawCube((Vector3){pos.x - 0.2f, pos.y + legHeight/2, pos.z - 0.35f}, 0.1f, legHeight, 0.1f, cowBody);
    DrawCube((Vector3){pos.x + 0.2f, pos.y + legHeight/2, pos.z - 0.35f}, 0.1f, legHeight, 0.1f, cowBody);

    // Hooves
    DrawCube((Vector3){pos.x - 0.2f, pos.y + 0.03f, pos.z + 0.35f}, 0.1f, 0.06f, 0.1f, cowHooves);
    DrawCube((Vector3){pos.x + 0.2f, pos.y + 0.03f, pos.z + 0.35f}, 0.1f, 0.06f, 0.1f, cowHooves);
    DrawCube((Vector3){pos.x - 0.2f, pos.y + 0.03f, pos.z - 0.35f}, 0.1f, 0.06f, 0.1f, cowHooves);
    DrawCube((Vector3){pos.x + 0.2f, pos.y + 0.03f, pos.z - 0.35f}, 0.1f, 0.06f, 0.1f, cowHooves);

    // Udder
    DrawSphere((Vector3){pos.x, pos.y + 0.35f, pos.z - 0.1f}, 0.12f, cowPink);

    // Tail
    DrawCube((Vector3){pos.x, pos.y + 0.7f, pos.z - 0.6f}, 0.04f, 0.04f, 0.2f, cowBody);
    DrawCube((Vector3){pos.x, pos.y + 0.5f, pos.z - 0.7f}, 0.06f, 0.15f, 0.04f, cowSpots);

    if (highlighted) {
        Color outlineColor = { 255, 255, 0, 255 };
        DrawCubeWires((Vector3){pos.x, pos.y + 0.6f, pos.z}, 0.65f, 0.55f, 1.05f, outlineColor);
        DrawCubeWires((Vector3){pos.x, pos.y + 0.7f, pos.z + 0.65f}, 0.4f, 0.4f, 0.35f, outlineColor);
    }
}
...
void DrawEnemy(const Enemy& enemy, bool highlighted) {
    switch (enemy.type) {
        case ENEMY_TROLL:
            DrawTroll(enemy.position, highlighted);
            break;
        case ENEMY_COW:
            DrawCow(enemy.position, highlighted);
            break;
        default:
